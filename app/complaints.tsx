...
import { AlertTriangle, Plus, ChevronLeft, Search, Calendar, Eye, Users, RefreshCw } from 'lucide-react-native';
...
import { isManagementRole, canActionComplaints } from '../src/lib/permissions';
...
export default function ComplaintsScreen() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const [myRes, pendingRes, allRes] = await Promise.all(promises);

            const myBody = myRes.data as ApiEnvelope<ComplaintRow[]>;
            const pendingBody = pendingRes.data as ApiEnvelope<ComplaintRow[]>;
            const allBody = allRes.data as ApiEnvelope<ComplaintRow[]>;

            if (myBody.success) setMyComplaints(myBody.data || []);
            if (pendingBody.success) setPendingComplaints(pendingBody.data || []);
            if (allBody.success) setAllComplaints(allBody.data || []);
        ...
        <View className="flex-1 bg-white">
            ...
            <SafeAreaView className="flex-1 pb-4" edges={['top', 'left', 'right']}>
                ...
                <View className="px-6 py-4 flex-row items-center justify-between border-b border-neutral-100 bg-white/70">
                    ...
                    <TouchableOpacity
                        ...
                        className="p-2 rounded-xl border border-neutral-200 bg-white shadow-xs"
                    >
                        ...
                        <View className="flex-row rounded-2xl bg-neutral-100 p-1">
                            <TouchableOpacity
                                ...
                                className={`flex-1 py-2.5 items-center justify-center rounded-xl ${
                                    scopeMode === 'team' ? 'bg-white shadow-sm' : ''
                                }`}
                            >
                            ...
                            <TouchableOpacity
                                ...
                                className={`flex-1 py-2.5 items-center justify-center rounded-xl ${
                                    scopeMode === 'my' ? 'bg-white shadow-sm' : ''
                                }`}
                            >
                                <Text className={`text-[10px] font-black uppercase tracking-wider ${
                            ...
                            <TouchableOpacity
                                ...
                                className={`flex-1 py-2.5 items-center justify-center rounded-xl ${
                                    scopeMode === 'all' ? 'bg-white shadow-sm' : ''
                                }`}
                            >
                                <Text className={`text-[10px] font-black uppercase tracking-wider ${
                            ...
                            <TouchableOpacity
                                ...
                                className={`flex-1 py-2.5 items-center justify-center rounded-xl ${
                                    scopeMode === 'history' ? 'bg-white shadow-sm' : ''
                                }`}
                            >
                                <Text className={`text-[10px] font-black uppercase tracking-wider ${
                ...
                <View className="px-6 mt-4 flex-row items-center gap-3">
                    {/* Search Field */}
                    <View className="flex-1 flex-row items-center bg-white rounded-2xl border-2 border-neutral-100 px-3 py-1.5 shadow-xs">
                        ...
                        <TextInput
                            ...
                            className="ml-2 flex-1 h-8 text-xs font-semibold text-neutral-800"
                        />
                    </View>

                    {/* Status Toggle Modal/Dropdown */}
                    <ScrollView 
                        horizontal 
                        showsHorizontalScrollIndicator={false}
                        className="grow-0 shrink-0 max-w-[45%]"
                    >
                    ...
                    <View className="mt-3">
                        ...
                        >
                            ...
                            <TouchableOpacity
                                ...
                                className={`flex-row items-center gap-1.5 px-3.5 py-2.5 rounded-xl border text-[10px] font-bold ${
                                    !selectedCategory
                                        ? 'border-rose-300 bg-rose-50/50 text-rose-800 font-extrabold shadow-xs'
                                        : 'border-neutral-100 bg-neutral-50 text-neutral-600'
                                }`}
                            >
                                <View className="h-1.5 w-1.5 rounded-full bg-rose-500" />
                                <Text className="text-[10px] font-bold">All Categories</Text>
                            ...
                            {Object.entries(categoryAggregates.counts).map(([cat, count]) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <Text className="text-[10px] font-bold">{cat}</Text>
                                        <View className={`px-1.5 py-0.5 rounded-md ${
                                    ...
                                    return (
                                        ...
                                        >
                                            <TouchableOpacity
                                                ...
                                                className="mb-3.5 rounded-3xl border border-neutral-100 bg-white p-5 shadow-xs"
                                            >
            ...
            {scopeMode === 'my' && (
                <TouchableOpacity
                    ...
                    className="absolute bottom-28 right-6 h-14 w-14 items-center justify-center rounded-full bg-rose-600 shadow-lg shadow-rose-300"
                >
